...
global conn,cursor
conn = mysql.connector.connect(host='db4free.net ',database='shreyaanand',user='shreyaanand',password='12345678')
cursor = conn.cursor()
def clear():
  for _ in range(5):
     print()
...
def add_crime_type():
    ...
    sql = 'insert into crime_type(Id,offence_name,ipc_section,comment) values({},"{}","{}","{}");'.format(Id,offence_name,ipc_section,comment)
    cursor.execute(sql)
    conn.commit()
